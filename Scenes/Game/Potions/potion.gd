...
func _on_combining_area_area_entered(area: Area3D) -> void:
	if area.get_parent() is Potion:
		...
		if potion.correct_potion and potion.potion_color == "RED":
		...
		if potion.correct_potion and potion.potion_color == "BLUE":
		...
		if correct_potions == 2:
			...
			var swirl_material: ShaderMaterial = ShaderMaterial.new()
			swirl_material.shader = load("res://Shaders/swirl.gdshader")
			purple_material.next_pass = swirl_material
			mesh_instance_3d.set_surface_override_material(1, swirl_material)
